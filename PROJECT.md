...
CalendarSettings  — wakeHour, sleepHour, snapMinutes, defaultDuration,
                    ...
                    todoPanelTitle?, collapsedTodoTabIds?,
...
```
accounts           — one row per auth user; settings JSON (including synchronized to-do panel preferences), quote data, workspace revision and logical-usage metadata
...
- **Cross-midnight timed blocks** — Drag creation can continue into later day columns. The block remains one event anchored to its start date; `end` stores elapsed decimal hours from that date and may exceed 24 (for example, 11 PM–1 AM is `start: 23, end: 25`). Week/day and month views render a segment on each covered date, while selection, movement, resizing, recurrence, inspector edits, and persistence operate on the single block. Date-scoped analytics use the same midnight segments: each date receives only its local slice, range edges include spill-in and clip spill-out, and unique-hour totals merge overlaps independently per calendar date.
- **All-day blocks** — All-day blocks are always visible in both Plan and Actual and are excluded from Plan-to-Actual copying. Their inspector and right-click menu therefore omit the layer-copy action while preserving the ordinary action-grid geometry with an empty reserved slot; Duplicate remains available when a distinct all-day event is intended. In month view they use a filled category-colored treatment and sort ahead of timed events, ensuring they receive priority in the visible event rows. Hovering only a day's date-header row reveals a muted plus beside the day number; it creates an all-day draft in the current layer, which then follows the shared draft/inspector lifecycle and remains visible in both layers. All-day blocks reuse `EventCard` and `EventMenu`, never render resize controls, can move between days, and can be reordered within a day; their hidden start-minute value stores that visual order. In week/day views, dragging an all-day block into the timed grid converts it to a timed block at the snapped drop time using the configured default duration, while dragging a timed block into an all-day slot converts it to all-day and places it at the indicated order.
...
- **Favorite calendars** — A calendar can be favorited or unfavorited from its right-click menu. Favorite IDs persist in settings, are removed on calendar deletion, and transfer to the merge target when a favorite calendar is merged. Month cells render each event once in the order all-day, favorite timed, then ordinary timed; the overflow tooltip remains neutral with all-day events followed by all timed events chronologically. In both Plan and Actual month views, an unmatched favorite timed event from the opposite layer remains visible until its end time; a source-linked counterpart or the same natural date/title match suppresses that fallback. All-day and favorite timed events share the filled category-color highlight without a left accent bar. The same menu can mark a calendar Ignored to toggle its inclusion in Weekly Insights; default, favorite, and ignore actions explain their effects with the shared accessible, viewport-aware calendar tooltip behavior.
...
- **Form controls** — Simple option menus use the shared app-themed `CalendarDropdown` rather than browser-native selects, keeping dark/light surfaces, selected states, and keyboard behavior consistent with the calendar. Specialized calendar assignment continues to use `CalendarSelect` and its grouped colored rows. `CalendarDropdown`, `CalendarSelect`, and nested context submenus all use the same anchored viewport-placement primitive, which chooses the available side, clamps both axes, scrolls oversized content, and repositions on resize or ancestor scroll.
- **Floating surface stacking** — Menus, dropdowns, and tooltips always render above sidebar resize handles and receive pointer/hover input there. The desktop right panel does not create a stacking context that can trap its fixed overlays; its mobile stacking layer is retained only where resize handles are hidden.
- **Keyboard shortcuts** — `lib/calendar/shortcuts.ts` is the single registry for command IDs, labels, groups, defaults, key-event matching, and platform-aware plain-text display. Settings and the command menu's `Keyboard shortcuts` action open a recorder-based editor for every keyboard command; duplicate assignments are rejected, individual bindings can be disabled or reset, reset-all restores the registry defaults, and Escape, a second click, or any pointer press outside the active recorder exits recording. Shortcut rows follow the app's quiet, borderless row language and use only the shared accent underline while recording. Overrides are stored in synchronized `accounts.settings` JSON through the ordinary `commit()` path, so they follow the account without a schema migration. Only differences from defaults (plus explicit `null` disables) are stored. Dismissible shortcut surfaces are true toggles: Command menu, To-do, Search, Settings, and Keyboard shortcuts close when their active shortcut is invoked again; closing To-do also exits event-linking mode. Weekly Insights remains the default right panel, so its shortcut reveals it instead of attempting to close it. Every command-palette action has a default binding, including Shift+T for To-do and Ctrl/Cmd+, for Settings, and palette/header hints always render the resolved binding. Command surfaces use text rather than a platform-specific Command glyph.
- **Touchpad week navigation** — A predominantly horizontal two-finger touchpad gesture over Week view shifts the displayed window one day at a time in that direction. Wheel deltas are normalized across pixel, line, and page modes, accumulated outside React, and consumed at most once per animation frame with a viewport-aware threshold. Each step uses a short directional same-document View Transition with a reduced-motion/direct-update fallback, skips hidden weekends, and updates `settings.weekStartsOn` through the ordinary `commit()` path so the Start of week control changes live and persists across reloads and synchronized devices.
- **Day bounds** — The configured wake and sleep times shade unavailable hours and draw Daily-load-style dashed rules across the timed-event grid at both boundaries. The timed grid adds a viewport-sized bottom scroll buffer when needed so the configured wake time can always align with the top of the scroll pane, including tall or resized layouts, plus a small visual gutter below the grid.
...
└── CalendarApp.tsx              ← authenticated root; owns UI state and keeps the cached/empty workspace visible during locked hydration
    ...
    ├── FloatingMenuCore.ts      ← shared outside-click/Escape dismissal plus pointer- and anchor-based viewport positioning for floating menus, dropdowns, and submenus
    ...
    ├── InsightsPanel.tsx        ← default weekly stats panel; accessible from the command menu/shortcut; omits calendars excluded in settings from every metric; date-scoped totals and tooltips use midnight-split timed allocations; Daily load stacks the By calendar grouped order bottom-to-top
...
- `block-time.ts` — cross-midnight block clock conversion, per-day segmentation, date-scoped allocation, and unique-hour merging
...
- `supabase/migrations/20260804000000_sync_todo_panel_preferences.sql` — backfills and validates synchronized to-do panel title/collapse preferences in account settings
...
### Tests
- `npm test` runs persistent Node regression tests. Block-time tests cover midnight splitting, range spill-in/spill-out, per-date overlap merging, and all-day exclusion. Recurrence tests cover multi-day and daily generation, absolute schedule assignment, immutable canonical anchors, cross-day moves, mixed scoped edits/deletes, all pairs of successive following cuts, and all three-following permutations followed by all-events moves from every source occurrence. To-do tests cover timed allocation, recurring link scopes, hierarchy repair and filtering, recursive estimate rollups, subtree insertion/deletion, cross-tab moves, horizontal reparenting, cycle rejection, and synchronized panel preferences. Shortcut tests cover unique defaults, complete command-palette coverage, portable Ctrl/Cmd matching, customization/disable/reset behavior, outside-click recording dismissal, quiet-row styling, import normalization, and platform-aware text labels. Sync tests cover strict cursor validation, changed-row replacement, normalized task metadata/note independence, task-link cleanup on event deletion, tombstones, collapsed multi-revision pulls, two-browser convergence, same-row disjoint edits, same-field conflicts, both conflict choices, delete-versus-edit conflicts, nested settings merges, and object-order stability.
