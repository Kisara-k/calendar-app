...
export function mergeSnapshots(base:DatabaseSnapshot,local:DatabaseSnapshot,remote:DatabaseSnapshot):MergeResult{
  ...
  return{snapshot:{revision:remote.revision,account:mergeValue(base.account,local.account,remote.account,'account',conflicts) as DatabaseSnapshot['account'],groups:mergeRows(base.groups,local.groups,remote.groups,'groups',conflicts),calendars:mergeRows(base.calendars,local.calendars,remote.calendars,'calendars',conflicts),series:mergeRows(base.series,local.series,remote.series,'series',conflicts),blocks:mergeRows(base.blocks,local.blocks,remote.blocks,'blocks',conflicts),blockNotes:mergeRows(base.blockNotes??[],local.blockNotes??[],remote.blockNotes??[],'blockNotes',conflicts),todoTabs:mergeRows(base.todoTabs??[],local.todoTabs??[],remote.todoTabs??[],'todoTabs',conflicts),todoItems:mergeRows(base.todoItems??[],local.todoItems??[],remote.todoItems??[],'todoItems',conflicts),todoItemNotes:mergeRows(base.todoItemNotes??[],local.todoItemNotes??[],remote.todoItemNotes??[],'todoItemNotes',conflicts),todoLinks:mergeRows(base.todoLinks??[],local.todoLinks??[],remote.todoLinks??[],'todoLinks',conflicts)},conflicts}
}
