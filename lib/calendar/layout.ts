...
export type TimedEventVerticalPosition={top:number;bottom:number;height:number}

const overlaps=(a:CalendarBlock,b:CalendarBlock)=>a.start<b.end&&b.start<a.end
const overlapHours=(a:CalendarBlock,b:CalendarBlock)=>Math.max(0,Math.min(a.end,b.end)-Math.max(a.start,b.start))
const isForeground=(block:CalendarBlock,other:CalendarBlock)=>block.start>other.start||(block.start===other.start&&block.end<other.end)
...
export function timedEventVerticalPosition(start:number,end:number,hourHeight:number,dayHours=24):TimedEventVerticalPosition{return {top:start*hourHeight,bottom:(dayHours-end)*hourHeight,height:Math.max(1,(end-start)*hourHeight-2)}}

export function overlapLayout(blocks:CalendarBlock[],categoryOrder:Map<string,number>,overlayRatio=.75){const result=new Map<string,EventLayout>(),sorted=[...blocks].sort((a,b)=>a.start-b.start||(b.end-b.start)-(a.end-a.start)||(categoryOrder.get(a.categoryId)??Number.MAX_SAFE_INTEGER)-(categoryOrder.get(b.categoryId)??Number.MAX_SAFE_INTEGER)),groups:CalendarBlock[][]=[];sorted.forEach(block=>{const group=groups.find(items=>items.some(other=>overlaps(block,other)));if(group)group.push(block);else groups.push([block])});groups.forEach(group=>{const overlays=group.filter(block=>group.some(other=>other.id!==block.id&&isForeground(block,other)&&overlapHours(block,other)<=(other.end-other.start)*overlayRatio)),base=group.filter(block=>!overlays.includes(block));placeInColumns(base).forEach(([id,layout])=>result.set(id,{...layout,overlay:false}));placeInColumns(overlays,4,96).forEach(([id,layout])=>result.set(id,{...layout,overlay:true}))});return result}
