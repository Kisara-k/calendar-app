...
import { normalizeTodoHierarchy } from './todo'
...
export function normalizeCalendarData(value: CalendarData): CalendarData {
  ...
  const todoTabs=value.settings.todoTabs?.length?value.settings.todoTabs:[{id:'todo-inbox',name:'TO DO'}],todoTabIds=new Set(todoTabs.map(tab=>tab.id)),blockIds=new Set([...blocks,...(value.deletedCalendars??[]).flatMap(item=>item.blocks)].map(block=>block.id)),todoItems=normalizeTodoHierarchy((value.settings.todoItems??[]).filter(item=>todoTabIds.has(item.tabId)).map(item=>({...item,linkedBlockIds:(item.linkedBlockIds??[]).filter(id=>blockIds.has(id))})))
  return {...value,blocks,categories,groups:value.groups?.length?value.groups:[{id:'work',name:'WORK'},{id:'life',name:'LIFE'}],quoteBank:value.quoteBank?.length?value.quoteBank:['Shape the week before it shapes you.'],currentQuote:value.currentQuote??'Shape the week before it shapes you.',deletedCalendars:value.deletedCalendars??[],settings:{...value.settings,hourScale:value.settings.hourScale??1,monthScale:value.settings.monthScale??1,weekStartsOn:Number.isInteger(value.settings.weekStartsOn)&&value.settings.weekStartsOn>=0&&value.settings.weekStartsOn<=6?value.settings.weekStartsOn:0,planLabel:value.settings.planLabel??'Plan',actualLabel:value.settings.actualLabel??'Actual',autoFormatTitles:value.settings.autoFormatTitles??false,insightsExcludedCategoryIds:(value.settings.insightsExcludedCategoryIds??[]).filter(id=>categories.some(c=>c.id===id)),favoriteCategoryIds:(value.settings.favoriteCategoryIds??[]).filter(id=>categories.some(c=>c.id===id)),todoPanelTitle:value.settings.todoPanelTitle?.trim().slice(0,120)||'To-do list',collapsedTodoTabIds:(value.settings.collapsedTodoTabIds??[]).filter(id=>todoTabIds.has(id)),todoTabs,todoItems,keyboardShortcuts:normalizeShortcutOverrides(value.settings.keyboardShortcuts),defaultCategoryId:value.settings.defaultCategoryId&&categories.some(c=>c.id===value.settings.defaultCategoryId)?value.settings.defaultCategoryId:categories[0]?.id??''}}
}
...
export function loadEmptyCalendar(): CalendarData {
  ...
  return {...base,blocks:[],categories:[],groups:[],quoteBank:[],currentQuote:'',deletedCalendars:[],settings:{...base.settings,defaultCategoryId:'',todoPanelTitle:'To-do list',collapsedTodoTabIds:[],todoTabs:[{id:'todo-inbox',name:'TO DO'}],todoItems:[]}}
}
