...
const {adjacentVisibleDay,differenceInCalendarDays,formatTime,startOfWeek,toISO,weekDates,weekDayOrder}=require('../lib/calendar/date.ts')
...
test('adjacent visible days move one calendar day and skip hidden weekends',()=>{const friday=new Date(2026,6,17),monday=new Date(2026,6,20);assert.equal(toISO(adjacentVisibleDay(friday,1,true)),'2026-07-18');assert.equal(toISO(adjacentVisibleDay(friday,1,false)),'2026-07-20');assert.equal(toISO(adjacentVisibleDay(monday,-1,false)),'2026-07-17')})
