...
const {monthEventLayout,monthEventPriority,monthFavoriteFallbackIds}=require('../lib/calendar/month-layout.ts')
...
test('actual month includes only unmatched favorite planned events that have not ended',()=>{const favorites=new Set(['favorite']),planned=(id,date,start,end,categoryId='favorite')=>({...block(id,start,end),date,categoryId}),blocks=[planned('ongoing','2026-07-15',9,11),planned('future','2026-07-16',9,10),planned('passed','2026-07-15',7,9),planned('ordinary','2026-07-16',9,10,'work'),planned('copied','2026-07-16',10,11),planned('natural','2026-07-16',11,12),{...planned('copy','2026-07-16',10,11),id:'actual-copy',layer:'actual',sourcePlanId:'copied'},{...planned('natural','2026-07-16',11,12),id:'actual-natural',layer:'actual'}],ids=monthFavoriteFallbackIds(blocks,'actual',favorites,new Date(2026,6,15,10).getTime());assert.deepEqual([...ids],['ongoing','future'])})

test('plan month includes only unmatched favorite actual events that have not ended',()=>{const favorites=new Set(['favorite']),planned=(id,date,start,end,categoryId='favorite')=>({...block(id,start,end),date,categoryId}),actual=(id,date,start,end,categoryId='favorite')=>({...planned(id,date,start,end,categoryId),layer:'actual'}),blocks=[planned('linked-plan','2026-07-16',9,10),planned('natural','2026-07-16',10,11),actual('future-actual','2026-07-16',11,12),actual('passed-actual','2026-07-15',7,9),actual('ordinary-actual','2026-07-16',12,13,'work'),{...actual('linked-actual','2026-07-16',9,10),sourcePlanId:'linked-plan'},{...actual('natural','2026-07-16',10,11),id:'natural-actual'}],ids=monthFavoriteFallbackIds(blocks,'plan',favorites,new Date(2026,6,15,10).getTime());assert.deepEqual([...ids],['future-actual'])})
