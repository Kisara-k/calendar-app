...
test('all recurring scoped mutations default to one event and expose broader scopes plus undo in a toast',()=>{const app=fs.readFileSync(require.resolve('../components/calendar/CalendarApp.tsx'),'utf8'),store=fs.readFileSync(require.resolve('../hooks/useCalendarStore.ts'),'utf8');assert.match(app,/const commitId=store\.updateRecurringBlock\(original,next,'only'\)/);assert.match(app,/store\.deleteRecurringBlock\(single,'only'\)/);assert.ok(app.includes("action:'edit'|'move'|'resize'")&&app.includes("action:'delete'")&&app.includes("'Edited'")&&app.includes("'Moved'")&&app.includes("'Resized'")&&app.includes("'Deleted'")&&app.includes('This and following')&&app.includes('All events')&&app.includes('store.undoCommit(recurringChangeToast.commitId)'));assert.ok(!app.includes('RecurrenceScopeDialog')&&!app.includes('scopeRequest'));assert.match(store,/replaceCommitId\?: string/)});
