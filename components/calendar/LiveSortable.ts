...
export function useLiveSortable(input:string|{id:string}){const id=typeof input==='string'?input:input.id,sortable=useSortable({id}),{setNodeRef,transform,transition,isDragging}=sortable,liveTransform=transform?{...transform,x:0,scaleX:1,scaleY:1}:null;return{...sortable,transform:liveTransform,attributes:{...sortable.attributes,'data-live-sortable':'','data-live-dragging':isDragging||undefined},rootProps:{ref:setNodeRef,style:{transform:CSS.Transform.toString(liveTransform),transition} as CSSProperties}}}
