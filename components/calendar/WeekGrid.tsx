...
type Interaction =
  ...
  | {type:'move';pointerId:number;originX:number;originY:number;block:CalendarBlock;offset:number;grabDayOffset:number;visualStart:number;dateIndex:number;start:number;moved:boolean;openOnRelease:boolean}
...
export function WeekGrid({dates,blocks,categories,settings,layer,selectedIds,onSelect,onSelectMany,onClearSelection,onCreate,onUpdate,onUpdateMany,onOpen,onEventContext,onCopyPlanDay}:Props){
  ...
  const renderSegments=(items:CalendarBlock[])=>items.flatMap(block=>timedBlockSegments(block).filter(segment=>visibleDates.has(segment.date)).map(segment=>({...segment,block,key:`${block.id}:${segment.date}`})))
  ...
  function beginEvent(e:React.PointerEvent,kind:'move'|'resize',block:CalendarBlock,grabDayOffset=0,visualStart=block.start){
    ...
    if(kind==='move')setInteraction({type:'move',pointerId:e.pointerId,originX:e.clientX,originY:e.clientY,block,offset:p.time-visualStart,grabDayOffset,visualStart,dateIndex:p.day,start:block.start,moved:false,openOnRelease:!e.shiftKey&&!e.ctrlKey&&!e.metaKey&&!inMultiSelection})
  ...
  function move(e:React.PointerEvent){
    ...
    if(interaction.type==='move')setInteraction({...interaction,dateIndex:p.day,start:clamp(snapTime(interaction.block.start+p.time-interaction.offset-interaction.visualStart,settings.snapMinutes),0,24-settings.snapMinutes/60),moved})
  ...
  const live=preview(),liveSegments=live?renderSegments([live]):[];const now=new Date();const nowIndex=dates.findIndex(d=>toISO(d)===toISO(now));const nowTime=now.getHours()+now.getMinutes()/60
