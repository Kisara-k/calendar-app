...
type Interaction =
  ...
  | {type:'move';pointerId:number;originX:number;originY:number;block:CalendarBlock;offset:number;grabDayOffset:number;visualStart:number;dateIndex:number;start:number;moved:boolean;openOnRelease:boolean;openInLinkMode:boolean;dropAllDay:boolean;allDayOrderIndex:number}
...
type Props={dates:Date[];blocks:CalendarBlock[];categories:CalendarCategory[];settings:CalendarSettings;layer:Layer;selectedIds:string[];linking:boolean;onSelect:(id:string,additive:boolean)=>void;onSelectMany:(ids:string[])=>void;onClearSelection:()=>void;onCreate:(b:Omit<CalendarBlock,'id'>)=>CalendarBlock;onUpdate:(b:CalendarBlock,action:'move'|'resize')=>void;onUpdateMany:(b:CalendarBlock[])=>void;onOpen:(id:string,keepLinking?:boolean)=>void;onEventContext:(id:string,x:number,y:number)=>void;onCopyPlanDay:(date:string)=>void}

const clamp=(n:number,min:number,max:number)=>Math.max(min,Math.min(max,n))

export function WeekGrid({dates,blocks,categories,settings,layer,selectedIds,linking,onSelect,onSelectMany,onClearSelection,onCreate,onUpdate,onUpdateMany,onOpen,onEventContext,onCopyPlanDay}:Props){
  ...
  function beginAllDay(e:React.PointerEvent,_kind:'move'|'resize',block:CalendarBlock){
    ...
    if(!linking){if(e.shiftKey||e.ctrlKey||e.metaKey)onSelect(block.id,true);else if(!selectedIds.includes(block.id))onSelect(block.id,false)}
  ...
  function beginEvent(e:React.PointerEvent,kind:'move'|'resize',block:CalendarBlock,grabDayOffset=0,visualStart=block.start){
    ...
    else if(!linking){if(e.shiftKey||e.ctrlKey||e.metaKey)onSelect(block.id,true);else if(!selectedIds.includes(block.id))onSelect(block.id,false)}
    const inMultiSelection=selectedIds.includes(block.id)&&selectedIds.length>1
    if(kind==='move')setInteraction({type:'move',pointerId:e.pointerId,originX:e.clientX,originY:e.clientY,block,offset:p.time-visualStart,grabDayOffset,visualStart,dateIndex:p.day,start:block.start,moved:false,openOnRelease:linking||!e.shiftKey&&!e.ctrlKey&&!e.metaKey&&!inMultiSelection,openInLinkMode:linking,dropAllDay:false,allDayOrderIndex:0})
  ...
  function end(e:React.PointerEvent){
    ...
    if(interaction.type==='move'&&!interaction.moved&&interaction.openOnRelease)onOpen(interaction.block.id,interaction.openInLinkMode&&e.shiftKey)
  ...
  return <div className="calendar-surface" style={{'--scrollbar-width':`${scrollbarWidth}px`} as React.CSSProperties}>
    ...
    <div className="time-scroll" ref={scrollRef}>
      ...
      <div aria-hidden="true" style={{height:bottomPadding+12}}/>
    </div>
