...
export function EventInspector({block,isDraft,categories,groups,onChange,onDelete,onDuplicate,onCopyLayer,onClose}:Props){const [title,setTitle]=useState(block.title),originalTitle=useRef(block.title);useEffect(()=>setTitle(block.title),[block.id,block.title]);useEffect(()=>{originalTitle.current=block.title},[block.id]);const patch=(p:Partial<CalendarBlock>)=>onChange({...block,...p});return <aside className="context-panel event-inspector">
  ...
  <div className="inspector-body"><input className="event-title-input" aria-label="Event title" placeholder="Add title" value={title} autoFocus={isDraft} onFocus={()=>{originalTitle.current=block.title}} onChange={e=>{const next=e.target.value;setTitle(next);patch({title:next})}} onKeyDown={e=>{if(e.key==='Enter'){e.preventDefault();e.currentTarget.blur()}if(e.key==='Escape'){e.preventDefault();e.stopPropagation();const previous=originalTitle.current;setTitle(previous);patch({title:previous});e.currentTarget.blur()}}}/>
