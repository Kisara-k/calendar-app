...
export function InsightsPanel({blocks,categories,groups,settings,dates,layer,onClose}:Props){
  ...
  const perCategory=insightCategories.map(c=>({c,plan:uniqueHours(plan.filter(b=>b.categoryId===c.id)),actual:uniqueHours(actual.filter(b=>b.categoryId===c.id))}))
