...
export function CalendarApp(){
  ...
  useEffect(()=>{if(window.innerWidth<850)setSidebarOpen(false)},[])
  ...
  return <main className="tempo-app" style={{'--ghost-opacity':data.settings.underlayOpacity/100} as React.CSSProperties}>
    ...
    <div className="app-body">{sidebarOpen&&<Sidebar anchor={anchor} categories={data.categories} groups={data.groups} deletedCalendars={data.deletedCalendars??[]} defaultCategoryId={data.settings.defaultCategoryId} panel={utilityPanel} onAnchor={setAnchor} onToggleCategory={store.toggleCategory} onToggleGroup={store.toggleGroup} onReorder={store.reorderCategories} onRename={store.renameCategory} onAdd={store.createCategory} onColor={store.colorCategory} onDefault={store.setDefaultCategory} onDelete={store.deleteCategory} onRestore={store.restoreCategory} onMerge={store.mergeCategory} onNew={newBlock} onPanel={showUtility}/>}<section className="calendar-workspace"><CalendarToolbar layer={layer} quote={data.currentQuote} settings={data.settings} onQuote={store.setQuote} onNextQuote={store.nextQuote} onCopyPlan={copyPlan} onPatch={store.patchSettings}/>{view==='month'?<MonthView anchor={anchor} blocks={renderedBlocks} categories={data.categories} layer={layer} onOpen={open} onDay={d=>{setAnchor(d);setView('day')}}/>:<WeekGrid dates={dates} blocks={renderedBlocks} categories={data.categories} settings={data.settings} layer={layer} selectedIds={selectedIds} onSelect={select} onClearSelection={()=>setSelectedIds([])} onCreate={createDraft} onUpdate={store.updateBlock} onUpdateMany={store.updateBlocks} onOpen={open} onEventContext={(id,x,y)=>{setSelectedIds([id]);setEventMenu({id,x,y})}}/>}</section>
