...
import type { CalendarBlock, Layer, Panel, ViewMode } from '@/lib/calendar/types'
...
export function CalendarApp(){
  ...
  const [layer,setLayer]=useState<Layer>('plan');const [view,setView]=useState<ViewMode>('week');const [anchor,setAnchor]=useState(new Date(2026,6,11));const [panel,setPanel]=useState<Panel>('insights');const [sidebarOpen,setSidebarOpen]=useState(true);const [selectedIds,setSelectedIds]=useState<string[]>([]);const [commandOpen,setCommandOpen]=useState(false);const [toast,setToast]=useState('')
  ...
  const open=useCallback((id:string)=>{setSelectedIds([id]);setPanel('event')},[])
  ...
  const deleteSelected=useCallback(()=>{if(!selectedIds.length)return;store.deleteBlocks(selectedIds);setSelectedIds([]);setPanel(null)},[selectedIds,store])

  useEffect(()=>{if(window.innerWidth<850){setSidebarOpen(false);setPanel(null)}},[])
  ...
  useEffect(()=>{
    const handler=(e:KeyboardEvent)=>{const target=e.target as HTMLElement;if(['INPUT','TEXTAREA','SELECT'].includes(target.tagName))return
      ...
      if(e.key==='?'){setPanel('shortcuts');return}if(e.key==='Escape'){setCommandOpen(false);setSelectedIds([]);if(panel==='event')setPanel(null);return}
      if(e.key==='Delete'||e.key==='Backspace'){deleteSelected();return}if(e.key==='ArrowLeft')navigate(-1);if(e.key==='ArrowRight')navigate(1)
      if(e.key.toLowerCase()==='n')newBlock();if(e.key.toLowerCase()==='t')setAnchor(new Date());if(e.key.toLowerCase()==='d')setView('day');if(e.key.toLowerCase()==='w')setView('week');if(e.key.toLowerCase()==='m')setView('month');if(e.key==='1')setLayer('plan');if(e.key==='2')setLayer('actual');if(e.key.toLowerCase()==='i')setPanel('insights');if(e.key==='/'){e.preventDefault();setPanel('search')}
    };window.addEventListener('keydown',handler);return()=>window.removeEventListener('keydown',handler)
  },[deleteSelected,navigate,newBlock,panel])

  const actions=[
    {label:'Create new block',hint:'N',icon:<CalendarDays size={15}/>,run:newBlock},{label:'Go to today',hint:'T',icon:<CalendarDays size={15}/>,run:()=>setAnchor(new Date())},{label:'Switch to Plan',hint:'1',icon:<Target size={15}/>,run:()=>setLayer('plan')},{label:'Switch to Actual',hint:'2',icon:<Check size={15}/>,run:()=>setLayer('actual')},{label:'Show weekly insights',hint:'I',icon:<BarChart3 size={15}/>,run:()=>setPanel('insights')},{label:'Search blocks',hint:'/',icon:<Search size={15}/>,run:()=>setPanel('search')},{label:'Open settings',hint:'',icon:<Settings2 size={15}/>,run:()=>setPanel('settings')},
  ]
  const closePanel=()=>setPanel(null)
  return <main className="tempo-app" style={{'--ghost-opacity':data.settings.underlayOpacity/100} as React.CSSProperties}>
    <AppHeader layer={layer} view={view} range={rangeLabel(dates)} sidebarOpen={sidebarOpen} panel={panel} onLayer={v=>{setLayer(v);setSelectedIds([]);if(panel==='event')setPanel(null)}} onView={setView} onToggleSidebar={()=>setSidebarOpen(v=>!v)} onNavigate={navigate} onToday={()=>setAnchor(new Date())} onPanel={setPanel} onCommand={()=>setCommandOpen(true)}/>
    <div className="app-body">{sidebarOpen&&<Sidebar anchor={anchor} categories={data.categories} panel={panel} onAnchor={setAnchor} onToggleCategory={store.toggleCategory} onNew={newBlock} onPanel={setPanel}/>}<section className="calendar-workspace">
      <CalendarToolbar layer={layer} subtitle={layer==='plan'?'Shape the week before it shapes you.':'Planned blocks stay visible as a quiet guide.'} onCopyPlan={copyPlan} onZoom={n=>store.patchSettings({density:n<0?'compact':'comfortable'})} onDisplay={()=>setPanel('settings')}/>
      ...
      {panel==='event'&&selected&&<EventInspector block={selected} categories={data.categories} onSave={store.updateBlock} onDelete={deleteSelected} onDuplicate={()=>{const b=store.createBlock({...selected,title:`${selected.title} copy`});open(b.id)}} onCopyLayer={target=>{const b=store.createBlock({...selected,layer:target,sourcePlanId:target==='actual'&&selected.layer==='plan'?selected.id:undefined});setLayer(target);open(b.id)}} onClose={closePanel}/>} 
      {panel==='insights'&&<InsightsPanel blocks={data.blocks} categories={data.categories} settings={data.settings} dates={dates} layer={layer} onClose={closePanel}/>} {panel==='settings'&&<SettingsPanel settings={data.settings} data={data} onPatch={store.patchSettings} onImport={next=>{try{store.replaceData(next);setToastBrief('Calendar imported')}catch{setToastBrief('That calendar file is not valid')}}} onReset={()=>{store.reset();setToastBrief('Demo calendar restored')}} onClose={closePanel}/>} {panel==='search'&&<SearchPanel blocks={data.blocks} categories={data.categories} onOpen={b=>{setAnchor(new Date(`${b.date}T12:00:00`));setLayer(b.layer);open(b.id)}} onClose={closePanel}/>} {panel==='shortcuts'&&<ShortcutsPanel onClose={closePanel}/>} 
    </div>
